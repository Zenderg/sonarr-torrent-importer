...
На дату ревью в репозитории не было реализации. Поэтому ни одно интеграционное предположение из концепта пока не доказано локальным прототипом; после появления прототипа эти выводы нужно сверить с фактическим поведением поддерживаемых версий Sonarr и qBittorrent.
