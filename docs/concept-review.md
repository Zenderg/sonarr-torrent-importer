...
Этот документ сохраняет историческое ревью исходной концепции и ранние ограничения стадии проектирования. Он не является документацией уже реализованной системы: актуальное поведение описано в [`README.md`](../README.md), проверка — в [`development.md`](development.md), релизный контракт — в [`releases.md`](releases.md).
...
На дату исходного ревью в репозитории не было реализации. Результирующий production workflow теперь доказан локальным Compose E2E на Sonarr v4 и qBittorrent v5: importer переименовывает `[NN].mkv` через qBittorrent, Sonarr автоматически импортирует canonical file, а history, episode-file, queue finalization и active seeding проверяются как postconditions. Поддерживаемая граница и команды проверки описаны в [`development.md`](development.md).
